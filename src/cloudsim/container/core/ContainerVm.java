...
import cloudsim.container.containerProvisioners.ContainerBwProvisioner;
import cloudsim.container.containerProvisioners.ContainerPe;
import cloudsim.container.containerProvisioners.ContainerRamProvisioner;
import cloudsim.container.lists.ContainerPeList;
import cloudsim.container.schedulers.ContainerScheduler;
...
//            Log.formatLine("Total mips usage is %.2f", requestedMipsTemp);
    ...
    public void setSize(long size) {
        //Log.printConcatLine("[VM ", getId(), "] # setSize = ", size);
        this.size = size;
    }
...
//        Log.printLine("Host: Create VM???......" + container.getId());
        if (getSize() < container.getSize()) {
            //Log.printConcatLine("[getSize container.getSize] #", getSize() + " " + container.getSize());
