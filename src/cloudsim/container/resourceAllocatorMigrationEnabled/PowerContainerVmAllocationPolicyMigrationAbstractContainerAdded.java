...
import cloudsim.container.lists.PowerContainerList;
...
public abstract class PowerContainerVmAllocationPolicyMigrationAbstractContainerAdded extends PowerContainerVmAllocationPolicyMigrationAbstract {
    ...
    public List<Map<String, Object>> optimizeAllocation(List<? extends ContainerVm> vmList) {
        ...
        HashSet<ContainerHost> excludedHostList = new HashSet<ContainerHost>(overUtilizedHosts);
        List<Map<String, Object>> migrationMap = getPlacementForLeftContainers(containersToMigrate, excludedHostList);

        //getExecutionTimeHistoryVmReallocation().add(ExecutionTimeMeasurer.end("optimizeAllocationVmReallocation"));
        //Log.printLine();

        Collection<? extends Map<String, Object>> migrationMapUnderUtilizedHosts = getContainerMigrationMapFromUnderUtilizedHosts(overUtilizedHosts, migrationMap);
        migrationMap.addAll(migrationMapUnderUtilizedHosts);
...
//                Add the host to the placement founder option
    ...
    protected List<Map<String, Object>> getNewContainerPlacement(List<? extends Container> containersToMigrate, Set<? extends ContainerHost> excludedHosts) {
    ...
    protected List<Map<String, Object>> getPlacementForLeftContainers(List<? extends Container> containersToMigrate, Set<? extends ContainerHost> excludedHostsList) {
...
//                    vm.setInWaiting(true);
    ...
    protected ContainerVm createVMinHost(ContainerHost host, boolean vmStatus) {
        for (int i = 0; i < numberOfVmTypes; i++) {
            ...
            if (getUtilizationOfCpuMips((PowerContainerHost) host) != 0 && isHostOverUtilizedAfterAllocation((PowerContainerHost) host, vm)) {
                //Log.printLine("Alloction will let host over utilization");
                continue;
            }
    ...
    protected List<Map<String, Object>> findMapInSwitchedOffHosts(List<Container> containerList) {
        Log.printLine(String.format(" %s :  Find Placement in the switched of hosts", CloudSim.clock()));
